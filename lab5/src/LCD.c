...
#define C_DINO                ((uint16_t) 0xB301)

#define C_DINOJUMP            ((uint16_t) 0xEC40)

#define C_OBSTACLE            ((uint16_t) 0x0181)
...
void LCD_Configure(void){
  ...
  LCD->CR &= ~(LCD_CR_DUTY);
  LCD->CR |= (LCD_CR_DUTY_0 | LCD_CR_DUTY_1); // Duty -> 011
...
uint8_t to_seconds(int millis) {
  return millis / 1000;
}
void LCD_DisplayTime(uint32_t millis) {
  uint8_t minutes = (millis / 60000);
  millis %= 60000;

  uint8_t seconds = (millis / 1000);
  millis %= 1000;

  uint8_t centiseconds = (millis / 10);

  uint8_t temp = minutes / 10;
  temp += 48;
  LCD_WriteChar(&temp, 0, 0, 0);
  temp = minutes % 10;
  temp += 48;
  LCD_WriteChar(&temp, 0, 1, 1);
  temp = seconds / 10;
  temp += 48;
  LCD_WriteChar(&temp, 0, 0, 2);
  temp = seconds % 10;
  temp += 48;
  LCD_WriteChar(&temp, 1, 0, 3);
  temp = centiseconds / 10;
  temp += 48;
  LCD_WriteChar(&temp, 0, 0, 4);
  temp = centiseconds % 10;
  temp += 48;
...
static void LCD_Object(uint8_t *digit, uint8_t object) {
  ...
  if(object == DINOJUMP)
    ...
    ch = C_DINO;
  else if (object == OBSTACLEDINO)
    ch = C_OBSTACLE | C_DINOJUMP;
  else
    ch = C_OBSTACLE;
